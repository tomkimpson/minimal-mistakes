...
for density of particles in the detector, $$n$$, and interaction cross section, $$\sigma$$, which quantifies the likelihood of a scattering event.

It can be shown (pg. 3) that this criterion approximately reduces to,

$$ f_{\gamma} \sigma M_d \frac{M_s}{R^2} \frac{1}{\epsilon_{\gamma}} >1$$

for graviton flux fraction $$ f_{\gamma} = L_{\gamma}/L$$. This seems to make intuitive sense - we want the flux/cross-section and masses of the detector and source to be as large as possible, and also want to minimize the distance between the source and detector. We then want to determine $$ f_{\gamma}$$ and $$ \sigma$$ for a given process.




### Cross section, $$ \sigma$$
The cross section is an effective area that sets how likely it is that a scattering event will take place when a beam is incident on an object composed of particles. It can also be thought of as the _rate_ of an event; high cross section, high rate of scattering. Typically in physics we talk of the differential cross section $$ d \sigma /d\Omega$$, which tells us the rate of scattering at a given angle ($$ \Omega$$ here is the [solid angle](https://en.wikipedia.org/wiki/Solid_angle) ). We can define this basically as,
...
<figure>
    <img src='https://upload.wikimedia.org/wikipedia/commons/thumb/2/2b/Jupiter_and_its_shrunken_Great_Red_Spot.jpg/800px-Jupiter_and_its_shrunken_Great_Red_Spot.jpg' alt='Jupiter' align="right" style="width:304px;height:228px;" />
    <figcaption>third Caption goes here</figcaption>
</figure>
